...
const TABLE_HEAD = [
  { id: 'numberTable', label: 'Bảng', alignRight: false },
  { id: 'price', label: 'Giá', alignRight: false },
  { id: 'status', label: 'Trạng thái', alignRight: false },
  { id: 'order', label: 'Đơn hàng', alignRight: false },
  { id: 'takeMoney', label: 'Nhận tiền', alignRight: false },
  { id: 'placed', label: 'Đặt hàng', alignRight: false },
  { id: 'chef', label: 'Bếp', alignRight: false },
  { id: '' },
];
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <div style={{display:'flex', flexDirection:'row'}}>
            <TextField onChange={handleChangeDate} value={valueDate} id="date" label="tìm theo ngày tháng năm" type="date" InputLabelProps={{ shrink: true }} sx={{ width: 220 }} />
                    ...
                    return (
                      <TableRow hover tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell align="left">{
                          item.status === "DON_MOI"
                            ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: '#82e8a7' }}>Mới</div>
                            : item.status === "DON_DA_XAC_NHAN"
                              ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: '#facc17' }}>Chờ bếp</div>
                              : item.status === "BEP_XAC_NHAN" 
                              ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: '#fb9948' }}>Nhân viên nhận món</div>
                              : item.status === "NHAN_VIEN_NHAN_MON" 
                              ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: '#1f6991' }}>Xong!</div>
                              : item.status === "HUY_DON" 
                              ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: 'red' }}>Hủy hóa đơn</div>
                              : (( item.status === "DA_THANH_TOAN") && item.isFailBill == true)
                              ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: '#f0cb54' }}>Hóa đơn thất bại đã nhận tiền</div>
                              : ((item.status === "FAIL_BILL" && item.status !== "DA_THANH_TOAN") && item.isFailBill == true)
                              ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: '#f0cb54' }}>Hóa đơn đền</div>
                              : item.status === "DA_THANH_TOAN"
                              ? <div style={{ paddingTop: 7, paddingBottom: 7, fontWeight: 600, width: 'inherit',  color: '#308e53' }}>Đã nhận tiền</div>
                              :<div>not status</div>
                                  }
                        </TableCell>
                      ...
                      <TableCell align="center" colSpan={6} sx={{ py: 3 }}>
                        ...
                        >
                          ...
                          <Typography variant="body2">
                           Không tìm thấy cho kết quả &nbsp;
                            <strong>&quot;{filterName}&quot;</strong>.
                            <br /> Kiếm tra lại thông tin nhập.
                          </Typography>
                        </Paper>
                      </TableCell>
                    </TableRow>
