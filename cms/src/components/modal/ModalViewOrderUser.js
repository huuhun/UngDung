    ...
    >
      <Box sx={style}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
          <p style={{ fontWeight: 600, color: '#ee7638' }}>Tổng hóa đơn: {billUser?.total} </p>
          {billUser?.isBonous == true && month!=='Month' ? <p style={{fontWeight: 600, color: '#880808'}}>thưởng</p> :<></>}
          <p style={{ fontWeight: 600, color: '#ee7638' }}>Giá: {billUser?.price}</p>
        </div>
        <div style={{ flexDirection: 'row', display: 'flex', paddingBottom: 7, borderBottom: '1px solid #bec1c7' }}>
          <div style={{ width: 150, color: '#A0AEC0', fontWeight: 600 }}>Bảng</div>
          <div style={{ width: 150, color: '#A0AEC0', fontWeight: 600 }}>Trạng thái</div>
          <div style={{ width: 200, color: '#A0AEC0', fontWeight: 600 }}>Sản phẩm</div>
          <div style={{ width: 150, color: '#A0AEC0', fontWeight: 600 }}>Giá</div>
        </div>
        <div>
          {billUser?.allBill.map((item, index) => {
            return (
              ...
              >
                ...
                <div style={{ width: 150 }}>
                  {item.status === 'DON_MOI' ? (
                    <div style={{ fontWeight: 600, width: 'inherit', color: '#82e8a7' }}>Đơn mới</div>
                  ) : item.status === 'DON_DA_XAC_NHAN' ? (
                    <div style={{ fontWeight: 600, width: 'inherit', color: '#facc17' }}>Đợi bếp xác nhận</div>
                  ) : item.status === 'BEP_XAC_NHAN' ? (
                    <div style={{ fontWeight: 600, width: 'inherit', color: '#fb9948' }}>Nhân viên nhận món</div>
                  ) : item.status === 'NHAN_VIEN_NHAN_MON' ? (
                    <div style={{ fontWeight: 600, width: 'inherit', color: '#1f6991' }}>Xong!</div>
                  ) : item.status === 'HUY_DON' ? (
                    <div style={{ fontWeight: 600, width: 'inherit', color: 'red' }}>Xóa đơn</div>
                  ) : item.status ==='DA_THANH_TOAN' ? (
                    <div style={{ fontWeight: 600, width: 'inherit', color: '#308e53' }}>Đã nhận tiền</div>
                  )
                  : (
                    <div>not status</div>
                  )}
